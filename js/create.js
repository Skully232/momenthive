...
// js/create.js
// Event creation page — Supabase backend, no Firebase dependency.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (logoFile) {
        try {
          btnText.textContent = 'Uploading logo...';
          ...
          finalLogoUrl = logoDataUrl; // Base64 fallback if Cloudinary not configured
        }
      }

      btnText.style.display = 'none';

      // 2. Prepare event record for Supabase (snake_case to match Postgres schema)
      ...
      console.log(`Event ${eventId} saved to Supabase.`);
      ...
      alert(`Error creating event album. Please check your internet connection and try again.\n\nDetails: ${err.message}`);
    } finally {
